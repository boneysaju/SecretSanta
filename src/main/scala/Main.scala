...
object Main extends App {
  ...
  def restartApp(): Unit = userInputType(StdIn.readLine("\n\nApp was reset.\nInsert 1 to use the participants.txt file or 2 to enter participants manually\nEnter 1 or 2 here: "))

  def userInputType[T](userChoice: T): Unit = {
    userChoice match {
      case "1" => CSV.userChoosesCsv()
      case "2" => ManualInput.userChoosesManualInput()
      case _ => println("That is an invalid input - please type 1 for Uploading a CSV file or type 2 to enter participants manually")
